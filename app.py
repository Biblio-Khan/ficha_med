...
if not st.session_state.autenticado:
    ...
    aba_login, aba_cadastro, aba_recuperar = st.tabs(["Iniciar Sessão", "Criar Nova Conta", "Esqueci a senha"])
        ...
        if st.button("Enviar Código de Recuperação"):
            try:
                ...
                if response_raw.status_code == 200:
                    ...
                    # O erro de conexão é genérico para não expor detalhes técnicos
                    st.error("Não foi possível conectar ao servidor. Tente novamente.")
            
            except Exception:
                st.error("Erro interno ao processar a solicitação.")
...
```""")
        ...
        if st.session_state.fichas_lote:
            ...
            # --- CORREÇÃO: Defina as colunas AQUI antes de usá-las ---
            col1, col2 = st.columns(2)
            
            with col1:
                # Botão para o arquivo .mrc
                st.download_button(
                    label=f"📥 Baixar .mrc",
        ...
        if not pedidos_pendentes:
            ...
            for ped in pedidos_pendentes:
                with st.container(border=True):
                    ...
                    with col1:
                        st.markdown(f"**Pedido:** `{ped.get('id_pedido', 'Sem ID')}` | **Data:** {ped.get('data', 'Sem data')}")
                        st.markdown(f"**Usuário:** {ped.get('nome', 'Desconhecido')} ({ped.get('email', 'Sem email')})")
                        st.markdown(f"**Volume Solicitado:** `{ped.get('pacote', '0')}` Fichas")
