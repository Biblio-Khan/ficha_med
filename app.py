...
# Definição de utilizador temporário para os testes de créditos
st.session_state["logado"] = True
st.session_state["usuario_atual"] = "teste_medico@gmail.com"

if "creditos_ativos" not in st.session_state:
    st.session_state["creditos_ativos"] = 10  # Valor padrão inicial para poderes testar livremente

with st.sidebar:
    st.write(f"👤 Utilizador de Teste: `{st.session_state['usuario_atual']}`")
...
# =========================================================================
# 🌟 RECARGA AUTOMÁTICA EM BACKEND (GOOGLE SHEETS)
# =========================================================================
        ...
        if df is not None:
            ...
            if 'token' in df.columns and 'creditos' in df.columns:
                ...
                if email_chave in df['token'].values:
                    ...
                    st.success(f"🔄 Saldo sincronizado com a planilha: {saldo} créditos.")
                else:
                    st.error("❌ O e-mail de teste NÃO foi encontrado na coluna 'token' da planilha.")
            else:
...
# =========================================================================
# 🔄 FUNÇÃO: BUSCA TRADUZIDA NA API DO PUBMED
# =========================================================================
                ...
                with st.spinner("Processando débito na aba creditos_med..."):
                    try:
                        ...
                        if resposta_google.status_code == 200 and resposta_google.json().get("status") == "sucesso":
                            ...
                            st.warning("⚠️ Nota: O script da planilha não retornou sucesso, mas salvamos a ficha no lote para teste!")
                            st.session_state.lote_fichas.append(txt_ficha)
                            st.rerun()
                    except Exception as e:
                        st.error(f"❌ Erro de conexão com o script da Planilha: {e}")
                        # Salva de qualquer forma no lote para não travar os teus testes visuais
                        st.session_state.lote_fichas.append(txt_ficha)
                        st.rerun()
